...
def format_waktu(w: Waktu) -> str:
    d = w.get_dates()
    t = w.get_time()
    return f"{d.get_year():04d}/{d.get_month():02d}/{d.get_day():02d} {t.get_hour():02d}:{t.get_minute():02d}:{t.get_seconds():02d}"
...
def output_tabel(kendaraan_list: List[Kendaraan]):
    if not kendaraan_list:
        print("\nTidak ada data kendaraan.")
        return
    
    print("\n" + "="*150)
    print("SISTEM PARKIR - LAPORAN KENDARAAN".center(150))
    print("="*150)
    
    # Header tabel
    header = f"{'No':<5} {'No. Kendaraan':<15} {'Nama Pemilik':<20} {'Jenis':<10} {'Waktu Masuk':<20} {'Waktu Keluar':<20} {'Biaya Parkir':>15}"
    print(header)
    print("-"*150)
    ...
    for i, k in enumerate(kendaraan_list, 1):
        no_kendaraan = k.get_no_kendaraan()
        nama = k.get_pemilik().get_name()
        jenis = k.get_jenis_kendaraan().name
        waktu_masuk = format_waktu(k.get_waktu_masuk())
        waktu_keluar = format_waktu(k.get_waktu_keluar())
        biaya = hitung_biaya_parkir(k)
        total_biaya += biaya
        
        row = f"{i:<5} {no_kendaraan:<15} {nama:<20} {jenis:<10} {waktu_masuk:<20} {waktu_keluar:<20} Rp {biaya:>12,}"
        print(row)
    
    print("-"*150)
    print(f"{'TOTAL':<72} Rp {total_biaya:>12,}")
    print("="*150)
